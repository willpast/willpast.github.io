---
title: 认知 - Elastic Stack和场景方案
date: 2024-05-30 14:56:17
permalink: /pages/db-es-introduce-scene/
tags: 
  - null
author: 
  name: Cyan
...
![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/es-introduce-1-1.png)

由于Elastic X-Pack是面向收费的，所以我们不妨也把X-Pack放进去，看看哪些是由X-Pack带来的，在阅读官网文档时将方便你甄别重点：

![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/es-introduce-1-0.png)

### Beats

Beats是一个面向**轻量型采集器**
的平台，这些采集器可以从边缘机器向Logstash、ElasticSearch发送数据，它是由Go语言进行开发的，运行效率方面比较快。从下图中可以看出，不同Beats的套件是针对不同的数据源。

![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/es-introduce-2-0.png)

### Logstash

Logstash是**动态数据收集管道**
，拥有可扩展的插件生态系统，支持从不同来源采集数据，转换数据，并将数据发送到不同的存储库中。其能够与ElasticSearch产生强大的协同作用，后被Elastic公司在2013年收购。
...
ElasticSearch对数据进行**搜索、分析和存储**
，其是基于JSON的分布式搜索和分析引擎，专门为实现水平可扩展性、高可靠性和管理便捷性而设计的。
...
Kibana实现**数据可视化**
，其作用就是在ElasticSearch中进行民航。Kibana能够以图表的形式呈现数据，并且具有可扩展的用户界面，可以全方位的配置和管理ElasticSearch。
...
![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/es-introduce-2-1.png)

### beats+logstath+elasticsearch+kibana

![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/es-introduce-2-2.png)
...
![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/es-introduce-2-3.png)

### beats+MQ+logstash+elasticsearch+kibana

![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/es-introduce-2-4.png)
...
![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/es-introduce-2-5.png)

增加数据源，和使用MQ

![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/es-introduce-2-6.png)

### Metric收集和APM性能监控

![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/es-introduce-2-7.png)

### 多数据中心方案

通过冗余实现数据高可用

![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/es-introduce-2-8.png)

两个数据采集中心（比如采集两个工厂的数据），采集数据后的汇聚

![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/es-introduce-2-9.png)

数据分散，跨集群的搜索

![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/es-introduce-2-10.png)
