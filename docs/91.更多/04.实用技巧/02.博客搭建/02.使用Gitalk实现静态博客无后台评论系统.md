...
::: warning
在issues中需要添加如下内容：
1. GitHub issue 的标题，标签需要包含`「评论」`+`document.title`。
2. GitHub issue 的标签，标签的内容为`id`值和`options.lables`的值。
3. 标签中id值长度不能超过代码中`slice(-x) `x的值。
:::
本页面评论的标签值如下：

![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/blog-comment-label.png)  
