...
date: 2024-05-23 15:20:26
permalink: /pages/java-thread-x-juc-collection-ConcurrentHashMap/
categories:
  - Java
  - 多线程与并发
tags:
...
![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/java-thread-x-concurrent-hashmap-1.png)
...
![java8_concurrenthashmap](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/java-thread-x-concurrent-hashmap-2.png)
...
- ConcurrentHashMap中key 和 value 不能为空
...
7.  判断是否要将链表转换为红黑树，临界值和 HashMap 一样，也是 8
...
final V putVal(K key, V value, boolean onlyIfAbsent) {
    ...
    for (Node<K,V>[] tab = table;;) {
        ...
        // hash 居然可以等于 MOVED，这个需要到后面才能看明白，不过从名字上也能猜到，肯定是因为在扩容
            ...
            V oldVal = null;
            // 获取数组该位置的头节点的监视器锁，即使用 synchronized 加锁加入节点
            synchronized (f) {
                if (tabAt(tab, i) == f) {
...
从源码中可以发现 `ConcurrentHashMap` 的初始化是通过**自旋和 CAS** 操作完成的。里面需要注意的是**变量 `sizeCtl`** ，它的值决定着当前的初始化状态。
...
- 根据该位置处结点性质进行相应查找 
  - ~~如果该位置为 null，那么直接返回 null 就可以了~~，concurrentHashMap不能放入value为null的值
  - 如果该位置处的节点刚好就是我们需要的，返回该节点的值即可
  - 如果该位置节点的 hash 值小于 0，说明正在扩容，或者是红黑树，后面我们再介绍 find 方法
  - 如果以上 3 条都不满足，那就是链表，遍历查找之。
