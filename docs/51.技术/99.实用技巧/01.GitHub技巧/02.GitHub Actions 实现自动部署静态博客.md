...
其次，代码要部署在`gh-pages`分支上，操作步骤如下：`Settings`->`Pages`->`GitHub Pages`->`Build and deployment`->`Select branch`，选择`gh-pages`的`root`文件夹，如图所示：

![img](https://cdn.jsdelivr.net/gh/willpast/image/blog/ka_java/auto-deploy.png)
